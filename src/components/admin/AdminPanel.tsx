...
export function AdminPanel({ onPageChange }: AdminPanelProps) {
  ...
  React.useEffect(() => {
    const fetchPending = async () => {
      ...
      try {
        ...
        const response = await axios.get(API_ENDPOINTS.PENDING_ITEMS, {
  ...
  const handleApprove = async (itemId: string) => {
    try {
      const token = localStorage.getItem('rewear_token');
      await axios.put(API_ENDPOINTS.APPROVE_ITEM(itemId), {}, {
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
  ...
  const handleReject = async (itemId: string) => {
    ...
    try {
      const token = localStorage.getItem('rewear_token');
      await axios.put(API_ENDPOINTS.REJECT_ITEM(itemId), { reason }, {
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
      setPendingItems(items => items.filter(item => item._id !== itemId));
    } catch {
