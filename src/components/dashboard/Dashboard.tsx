...
export function Dashboard({ onPageChange }: DashboardProps) {
  ...
  const [purchases, setPurchases] = useState<Array<{ id: string; images: string[]; title: string; pointValue: number; }>>([]);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const itemsResponse = await axios.get(API_ENDPOINTS.ITEMS, {
          headers: token ? { Authorization: `Bearer ${token}` } : {}
        });

        if (itemsResponse.data.success) {
          const allItems = itemsResponse.data.data.items || [];
          const userOwnedItems = allItems.filter((item: ClothingItem) => item.uploaderId === user.id);
          setUserItems(userOwnedItems);
        }

        // TODO: Fetch real purchases and stats from API
        // For now, using placeholder data
        setPurchases([]);
        setStats({
          totalSwaps: 3, // TODO: Replace with real data
          pendingRequests: 2 // TODO: Replace with real data
        });
